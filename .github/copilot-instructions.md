...
- `llm_call(task_category=..., messages=..., images=..., metadata=...)` for LLM/VLM calls.

Treat this function as the existing backend router call exposed through the existing `model_router_client`. Do not implement it, wrap it, subclass it, replace it, create another client, or create an alternate router. Do not choose or instantiate any detector directly in generated tools.
...
For OCR or text extraction, call `llm_call(task_category="ocr", ...)` and let the backend choose the implementation.
